...
export class TemplatesController {
  ...
  @ApiQuery({
    ...
    type: String,
    required: false,
    description: 'User Liked the template (Feed) or not',
  })
  ...
  async getTopList(
    ...
    try {
      ...
      const data = await this.templatesService.findToptemplates(
        app_id,
        tag,
        currentPage,
        pageSize,
  ...
  async getTrendingList(
    ...
    try {
      ...
      const data = await this.templatesService.findTrendingtemplates(
        app_id,
        tag,
        currentPage,
        pageSize,
  ...
  async getRecentList(
    ...
    try {
      ...
      const data = await this.templatesService.findRecenttemplates(
        app_id,
        tag,
        currentPage,
        pageSize,
  ...
  async approveTemplates(@Body() template: any, @Res() response: Response) {
    try {
      ...
      const data =
        await this.templatesService.approveUserTemplate(template_ids);
      return response.status(data.StatusCode).json(data);
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async declineTemplates(@Body() template: any, @Res() response: Response) {
    try {
      ...
      const data =
        await this.templatesService.declineUserTemplate(template_ids);
      return response.status(data.StatusCode).json(data);
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
  ...
  async update(
    ...
    try {
      const result = await this.templatesService.update(
        id,
        updateTemplateDto,
        files,
      );
  ...
  async updatetag(
    ...
    try {
      const result = await this.templatesService.updatetagname(
        id,
        UpdateTagDto,
      );
      if (result) {
  ...
  findAll(
    ...
    @Query('searchApproved') searchApproved: string,
    @Query('currentPage') currentPage: number,
    @Query('pageSize') pageSize: number,
  ) {
    try {
