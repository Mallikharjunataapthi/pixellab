import { Controller, Post, Body, HttpStatus, Res, UseGuards, InternalServerErrorException } from '@nestjs/common';
...
@Public()
  ...
  async create(@Body() createLikeDto: CreateLikeDto,@Res() response: Response) {
    // check already liked
    try{
      const checkUser = await this.likesService.checkUserAndTemplateById(createLikeDto);
