...
import {
  TemplateUsersReport,
  TemplateUserReportSchema,
} from './schema/usertemplatescount.schema';
...
export class UserTemplatesService {
  constructor(
    ...
    private TemplateUsersReportModel: Model<TemplateUsersReport>,
  ...
  async create(
    ...
    try {
      const beforeImageFile = files.user_temp_before_image_url
        ? files.user_temp_before_image_url[0]
        : null;
      const afterImageFile = files.user_temp_after_image_url
        ? files.user_temp_after_image_url[0]
        : null;
      let UsertempalteObject = {};
      const beforeImageS3Response =
        await this.fileuploader.s3_upload(beforeImageFile);
      const afterImageS3Response =
        await this.fileuploader.s3_upload(afterImageFile);
      if (createUserTemplateDto?.user_temp_app_id.length == 24) {
        const appid = new Types.ObjectId(
          createUserTemplateDto.user_temp_app_id,
        );
        ...
        if (
          ...
          if (
            createUserTemplateDto.user_temp_template_id != undefined &&
            createUserTemplateDto.user_temp_template_id != null
          ) {
            const templateData = await this.templatesService.findOne(
              createUserTemplateDto.user_temp_template_id,
            );
            ...
            if (
              ...
              await this.templatesService.updatereused_count(
                createUserTemplateDto.user_temp_template_id,
                templateData.data.used_count,
              );
            } else {
              ...
              await this.templatesService.updatereused_count(
                new Types.ObjectId(createUserTemplateDto.user_temp_template_id),
                templateData.data.used_count,
              );
            }

            const userusedCount = {
              app_id: new Types.ObjectId(
                createUserTemplateDto.user_temp_app_id,
              ),
            ...
            UsertempalteObject = {
              ...
              user_id: new Types.ObjectId(
                createUserTemplateDto.user_temp_user_id,
              ),
              template_id: new Types.ObjectId(
                createUserTemplateDto.user_temp_template_id,
              ),
              original_template_id: new Types.ObjectId(
                createUserTemplateDto.user_temp_original_template_id,
              ),
              before_image_url: beforeImageS3Response
                ? beforeImageS3Response
                : null,
              after_image_url: afterImageS3Response
                ? afterImageS3Response
                : null,
          ...
          if (
            createUserTemplateDto.purchase_url != undefined &&
            createUserTemplateDto.purchase_url != null &&
            createUserTemplateDto.purchase_url != ''
          ) {
            UsertempalteObject = {
              ...UsertempalteObject,
              purchase_url: createUserTemplateDto.purchase_url,
            };
          }
          if (
            ...
            const newUsertempalteObject = {
              ...UsertempalteObject,
              base_image_path: createUserTemplateDto.base_image_path,
            };
            await this.templatesService.createUserTemplate(
              newUsertempalteObject,
            );
          } else {
  ...
  findAll(
    ...
    try {
      const data = this.templatesService.getUserTemplates(
        app_id,
        user_id,
        currentPage,
        pageSize,
  ...
  findAllChildTemplate(
    ...
    try {
      const data = this.templatesService.getChildTemplate(
        app_id,
        template_id,
        currentPage,
        pageSize,
