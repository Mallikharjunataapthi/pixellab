...
export class ReportTemplateService {
  constructor(
    ...
    private ReportTemplateSchemaSchemaModel: Model<ReportTemplate>,
  ) {}
  async create(createReportTemplateDto: CreateReportTemplateDto) {
    try {
      const objectIdAsString: string =
        createReportTemplateDto?.app_id?.toString() || '';
  ...
  async findAll(page: number = 0, pageSize: number = 10) {
    try {
      ...
      const data = await this.ReportTemplateSchemaSchemaModel.find({
        is_active: 1,
      })
        .sort({ updatedAt: -1 })
        .skip(skip)
      ...
      const totalrepports =
        await this.ReportTemplateSchemaSchemaModel.countDocuments({
          is_active: 1,
        });
      return {
        success: true,
  ...
  async update(id: string, updateReportTemplateDto: UpdateReportTemplateDto) {
    try {
      await this.ReportTemplateSchemaSchemaModel.updateOne(
        { _id: id },
