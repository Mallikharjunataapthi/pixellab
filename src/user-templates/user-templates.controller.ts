...
import { ApiTags, ApiOperation ,ApiExcludeEndpoint, ApiQuery, ApiConsumes, ApiBody } from '@nestjs/swagger';
...
export class UserTemplatesController {
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        aspect_ratio_x: { type: 'string', required:['false'] },
        aspect_ratio_y: { type: 'string',required:['false'] },
      },
    },
  })
  ...
  @ApiQuery({ name: 'app_id', type: String })
  @ApiQuery({ name: 'template_id', type: String, description: 'Original Template Id' })
  @ApiQuery({ name: 'pageSize', type: Number })
  @ApiQuery({ name: 'currentPage', type: Number })
