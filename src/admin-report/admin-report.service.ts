...
export class AdminReportService {
  constructor(
    ...
    @InjectModel(TemplateUsersReport.name) private UserTemplateReportModel: Model<TemplateUsersReport>
  ){}
 ...
 async MostUsedTemplates(page:number=0,pageSize:number=10) {
  ...
  async MostActiveUsers(page:number,pageSize:number,fromDatestring:string,toDatestring:string){
    try {
      const skip = (page - 1) * pageSize;
      ...
      if(fromDatestring && toDatestring){
        totaltemplates = await this.UserTemplateReportModel.countDocuments(filter);
      }else{
