...
export class TemplatesService {
  ...
  async create(
    ...
    files: {
      before_image_url?: Express.Multer.File[];
      after_image_url?: Express.Multer.File[];
    },
  ) {
    const beforeImageFile = files.before_image_url
      ? files.before_image_url[0]
      : null;
    const afterImageFile = files.after_image_url
      ? files.after_image_url[0]
      : null;
    ...
    try {
      ...
      const categoryName = await this.CategoryModel.findById(
        createTemplateDto.cat_id,
      );
  ...
  async update(
    ...
    const beforeImageFile = files
      ? files.before_image_url
        ? files.before_image_url[0]
        : null
      : null;
    const afterImageFile = files
      ? files.after_image_url
        ? files.after_image_url[0]
        : null
      : null;
    ...
    if (beforeImageFile) {
      const oldFilepath = await this.TemplateModel.findById(id).select(
        '-_id before_image_url',
      );
      if (oldFilepath) {
    ...
    try {
      ...
      if (
        ...
        updateTemplateDto.purchase_url != null
      ) {
        updateObject.purchase_url = updateTemplateDto.purchase_url;
      }
      if (
        updateTemplateDto.base_image_path != undefined &&
        updateTemplateDto.base_image_path != null
      ) {
    ...
    try {
      ...
      const result = await this.TemplateModel.find(filter)
        ...
        .populate({
          path: 'likedBy',
          match: {
            user_id: user_ids != '0' ? new Types.ObjectId(user_ids) : user_ids,
          },
          select: 'user_id',
        })
        .lean();
